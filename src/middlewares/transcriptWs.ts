...
export const createTranscript = async (transcriptData) => {
  const newTranscript = {
    ...
    is_live: "is_live" in transcriptData
...
export const updateTranscript = async (transcriptId, transcriptData) => {
  ...
  if (
    Object.keys(transcript).length === 0 && transcript.constructor === Object
  ) {
    throw new Error("Transcript not found");
  }

  const updatedTranscript = {
    name: transcriptData.name !== undefined
      ? String(transcriptData.name)
      : transcript.name,
    brand: transcriptData.brand !== undefined ? String(transcriptData.brand)
    : transcript.brand,
    is_live: transcriptData.is_live !== undefined
      ? Boolean(transcriptData.is_live)
      : transcript.is_live,
  };
